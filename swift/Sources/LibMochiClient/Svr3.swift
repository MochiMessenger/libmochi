...
public class Svr3Client {
    ...
    ///
    /// - Throws:
    ///   On error, throws a ``MochiError``. Expected error cases are
    ///   - `MochiError.networkError` for a network-level connectivity issue,
    ///     including connection timeout.
    ///   - `MochiError.networkProtocolError` for an SVR3 or attested
    ...
    ///     retry-with-backoff mechanism inside libmochi used to connect to the
    ...
    ///   - `MochiError.networkProtocolError` for an SVR3 or attested
    ///     connection protocol issue.
    ///   - `MochiError.svrDataMissing` when either the maximum number of
    ///     restores has been exceeded or the value has never been backed up in
    ///     the first place.
    ///   - `MochiError.svrRestoreFailed` when the restore failed due to a bad
    ...
    public func remove(auth: Auth) async throws {
        _ = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.connectionManager.withNativeHandle { connectionManager in
                mochi_svr3_remove(promise, asyncContext, connectionManager, auth.username, auth.password)
            }
        }
    }
}
