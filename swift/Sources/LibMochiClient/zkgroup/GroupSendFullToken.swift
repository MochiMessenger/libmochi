...
public class GroupSendFullToken: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_group_send_full_token_check_valid_contents)
    }
        ...
        let expirationSinceEpoch: UInt64 = failOnError {
            try withUnsafeBorrowedBuffer { response in
                try invokeFnReturningInteger {
                    mochi_group_send_full_token_get_expiration($0, response)
                }
            }
        }
    ...
    ///
    /// - Throws: ``MochiError/verificationFailed(_:)`` if the token is invalid
    public func verify(
        userIds: [ServiceId],
        now: Date = Date(),
        ...
        try withUnsafeBorrowedBuffer { token in
            try ServiceId.concatenatedFixedWidthBinary(userIds).withUnsafeBorrowedBuffer { userIds in
                try keyPair.withUnsafeBorrowedBuffer { keyPair in
                    try checkError(
                        mochi_group_send_full_token_verify(token, userIds, UInt64(now.timeIntervalSince1970), keyPair)
                    )
                }
            }
        }
