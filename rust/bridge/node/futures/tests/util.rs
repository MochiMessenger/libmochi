...
pub fn run(action: &str) {
    ...
    for possible_library_name in &[
        "libmochi_neon_futures_tests.so",
        "libmochi_neon_futures_tests.dylib",
        "mochi_neon_futures_tests.dll",
    ] {
    ...
    let node_library_path = std::env::temp_dir().join("mochi_neon_futures_tests.node");
    ...
    let status = Command::new("yarn")
        .arg(action)
        .env("MOCHI_NEON_FUTURES_TEST_LIB", &node_library_path)
    ...
    if !status.success() {
        eprintln!(
            "\nMOCHI_NEON_FUTURES_TEST_LIB={:?} yarn --cwd {:?} {}\n",
