...
public class CallLinkAuthCredential: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_call_link_auth_credential_check_valid_contents)
    }
                        ...
                        try callLinkParams.withUnsafeBorrowedBuffer { callLinkParams in
                            try randomness.withUnsafePointerToBytes { randomness in
                                try invokeFnReturningVariableLengthSerialized {
                                    mochi_call_link_auth_credential_present_deterministic($0, contents, userId, UInt64(redemptionTime.timeIntervalSince1970), serverParams, callLinkParams, randomness)
                                }
                            }
                        }
                    }
