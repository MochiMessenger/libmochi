...
public class MessageBackupKey: NativeHandleOwner {
    public convenience init(masterKey: [UInt8], aci: Aci) throws {
        ...
        let handle = try masterKey.withUnsafePointerToSerialized { masterKey in
            try aci.withPointerToFixedWidthBinary { aci in
                ...
                try checkError(mochi_message_backup_key_new(&outputHandle, masterKey, aci))
                return outputHandle
            }
        }
    ...
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        mochi_message_backup_key_destroy(handle)
    }
}
...
public func validateMessageBackup(
    key: MessageBackupKey, purpose: MessageBackupPurpose, length: UInt64, makeStream: () throws -> MochiInputStream
        ...
        try withInputStream(try makeStream()) { secondInput in
            try key.withNativeHandle { key in
                try invokeFnReturningNativeHandle {
                    mochi_message_backup_validator_validate($0, key, firstInput, secondInput, length, purpose.rawValue)
                }
            }
        }
    }
        ...
        let fields = failOnError {
            try self.withNativeHandle { result in
                try invokeFnReturningStringArray {
                    mochi_message_backup_validation_outcome_get_unknown_fields($0, result)
                }
            }
        }
    ...
    public var errorMessage: String? {
        try! self.withNativeHandle { result in
            try invokeFnReturningOptionalString {
                mochi_message_backup_validation_outcome_get_error_message($0, result)
            }
        }
    }

    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        mochi_message_backup_validation_outcome_destroy(handle)
    }
}
