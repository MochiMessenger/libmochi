...
public class ProfileKeyCredentialPresentation: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_profile_key_credential_presentation_check_valid_contents)
    }

    public func getUuidCiphertext() throws -> UuidCiphertext {
        return try withUnsafeBorrowedBuffer { buffer in
            try invokeFnReturningSerialized {
                mochi_profile_key_credential_presentation_get_uuid_ciphertext($0, buffer)
            }
        }
    }
    ...
    public func getProfileKeyCiphertext() throws -> ProfileKeyCiphertext {
        return try withUnsafeBorrowedBuffer { buffer in
            try invokeFnReturningSerialized {
                mochi_profile_key_credential_presentation_get_profile_key_ciphertext($0, buffer)
            }
        }
    }
}
