...
#
# Copyright (C) 2020-2022 Mochi Messenger, LLC.
...
while [ "${1:-}" != "" ]; do
    case "${1:-}" in
        desktop )
            ...
            echo_then_run cargo build -p libmochi-jni --release --features "${FEATURES[*]}"
            if [[ -z "${CARGO_BUILD_TARGET:-}" ]]; then
                copy_built_library target/release mochi_jni "${DESKTOP_LIB_DIR}/"
            fi
            exit
            ;;
        android )
...
for abi in "${android_abis[@]}"; do
    ...
    echo_then_run cargo build -p libmochi-jni --release ${FEATURES:+--features "${FEATURES[*]}"} -Z unstable-options --target "$rust_target" --out-dir "${ANDROID_LIB_DIR}/$abi"
done
