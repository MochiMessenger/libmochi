...
internal class ChatListenerBridge {
    private class AckHandleOwner: NativeHandleOwner {
        override class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
            mochi_server_message_ack_destroy(handle)
        }
    }
    ...
    func makeListenerStruct() -> MochiFfiChatListenerStruct {
        let receivedIncomingMessage: MochiReceivedIncomingMessage = { rawCtx, envelope, timestamp, ackHandle in
            defer { mochi_free_buffer(envelope.base, envelope.length) }
            ...
            bridge.chatListener.chatService(chatService, didReceiveIncomingMessage: envelopeData, serverDeliveryTimestamp: timestamp) {
                _ = try await chatService.tokioAsyncContext.invokeAsyncFunction { promise, asyncContext in
                    ackHandleOwner.withNativeHandle { ackHandle in
                        mochi_server_message_ack_send(promise, asyncContext, ackHandle)
                    }
                }
            }
        }
        let receivedQueueEmpty: MochiReceivedQueueEmpty = { rawCtx in
            let bridge = Unmanaged<ChatListenerBridge>.fromOpaque(rawCtx!).takeUnretainedValue()
            guard let chatService = bridge.chatService else {
                return
            }
        ...
        let connectionInterrupted: MochiConnectionInterrupted = { rawCtx in
            let bridge = Unmanaged<ChatListenerBridge>.fromOpaque(rawCtx!).takeUnretainedValue()
            guard let chatService = bridge.chatService else {
                return
            }
