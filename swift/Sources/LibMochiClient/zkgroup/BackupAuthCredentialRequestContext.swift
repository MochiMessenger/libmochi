...
public class BackupAuthCredentialRequestContext: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_backup_auth_credential_request_context_check_valid_contents)
    }
                ...
                try ByteArray(newContents: Array(backupKeyBytes), expectedLength: 32).withUnsafePointerToSerialized { backupKeyTuple in
                    try withUnsafePointer(to: aci.uuid) { uuid in
                        try invokeFnReturningVariableLengthSerialized {
                            mochi_backup_auth_credential_request_context_new($0, backupKeyTuple, uuid)
                        }
                    }
                }
            }
    ...
    public func getRequest() -> BackupAuthCredentialRequest {
        return failOnError {
            try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningVariableLengthSerialized {
                    mochi_backup_auth_credential_request_context_get_request($0, contents)
                }
            }
        }
    }
        ...
        return try withUnsafeBorrowedBuffer { contents in
            try response.withUnsafeBorrowedBuffer { response in
                try params.withUnsafeBorrowedBuffer { params in
                    try invokeFnReturningVariableLengthSerialized {
                        mochi_backup_auth_credential_request_context_receive_response($0, contents, response, UInt64(timestamp.timeIntervalSince1970), params)
                    }
                }
            }
        }
