                ...
                try aci.withPointerToFixedWidthBinary { aci in
                    try pni.withPointerToFixedWidthBinary { pni in
                        try invokeFnReturningVariableLengthSerialized {
                            mochi_server_secret_params_issue_auth_credential_with_pni_as_service_id_deterministic($0, serverSecretParams, randomness, aci, pni, redemptionTime)
                        }
                    }
                }
            }
        ...
        return try self.serverSecretParams.withNativeHandle { serverSecretParams in
            try randomness.withUnsafePointerToBytes { randomness in
                try aci.withPointerToFixedWidthBinary { aci in
                    try pni.withPointerToFixedWidthBinary { pni in
                        try invokeFnReturningVariableLengthSerialized {
                            mochi_server_secret_params_issue_auth_credential_with_pni_zkc_deterministic($0, serverSecretParams, randomness, aci, pni, redemptionTime)
                        }
                    }
                }
            }
        ...
        try self.serverSecretParams.withNativeHandle { serverSecretParams in
            try groupPublicParams.withUnsafePointerToSerialized { groupPublicParams in
                try authCredentialPresentation.withUnsafeBorrowedBuffer { authCredentialPresentation in
                    try checkError(mochi_server_secret_params_verify_auth_credential_presentation(serverSecretParams, groupPublicParams, authCredentialPresentation, UInt64(now.timeIntervalSince1970)))
                }
            }
        }
    }
