...
/// so we can't go from, say, `Int32` to `MochiCPromisei32` (a function taking `UnsafePointer<Int32>`,
...
internal protocol PromiseStruct {
    ...
    var cancellation_id: MochiCancellationId { get }
}

extension MochiCPromisebool: PromiseStruct {
    typealias Result = Bool
}

extension MochiCPromisei32: PromiseStruct {
    typealias Result = Int32
}

extension MochiCPromiseRawPointer: PromiseStruct {
    typealias Result = UnsafeRawPointer
}

extension MochiCPromiseCdsiLookup: PromiseStruct {
    typealias Result = OpaquePointer
}

extension MochiCPromiseFfiCdsiLookupResponse: PromiseStruct {
    typealias Result = MochiFfiCdsiLookupResponse
}

extension MochiCPromiseFfiChatResponse: PromiseStruct {
    typealias Result = MochiFfiChatResponse
}

extension MochiCPromiseFfiChatServiceDebugInfo: PromiseStruct {
    typealias Result = MochiFfiChatServiceDebugInfo
}

extension MochiCPromiseFfiResponseAndDebugInfo: PromiseStruct {
    typealias Result = MochiFfiResponseAndDebugInfo
}

extension MochiCPromiseOwnedBufferOfc_uchar: PromiseStruct {
    typealias Result = MochiOwnedBuffer
}
...
private class CompleterBase {
    let completeUnsafe: (_ error: MochiFfiErrorRef?, _ valuePtr: UnsafeRawPointer?) -> Void

    init(completeUnsafe: @escaping (MochiFfiErrorRef?, UnsafeRawPointer?) -> Void) {
...
/// so that it can be completed from a libmochi\_ffi Promise without needing
...
private class Completer<Promise: PromiseStruct>: CompleterBase {
        ...
        super.init { error, valuePtr in
            continuation.resume(with: Result {
                ...
                guard let valuePtr else {
                    throw MochiError.internalError("produced neither an error nor a value")
                }
    ...
    func makePromiseStruct() -> Promise {
        typealias RawPromiseCallback = @convention(c) (_ error: MochiFfiErrorRef?, _ value: UnsafeRawPointer?, _ context: UnsafeRawPointer?) -> Void
        ...
        typealias RawPointerPromiseCallback = @convention(c) (_ error: MochiFfiErrorRef?, _ value: UnsafePointer<UnsafeRawPointer?>?, _ context: UnsafeRawPointer?) -> Void
        let rawPromiseStruct = MochiCPromiseRawPointer(complete: unsafeBitCast(completeOpaque, to: RawPointerPromiseCallback.self), context: Unmanaged.passRetained(self).toOpaque(), cancellation_id: 0)
        ...
        precondition(MemoryLayout<MochiCPromiseRawPointer>.size == MemoryLayout<Promise>.size)
...
/// Provides a context struct for calling Promise-based libmochi\_ffi functions.
///
/// Example:
///
/// ```
/// let result: Int32 = try await invokeAsyncFunction {
///   mochi_do_async_work($0, someInput, someOtherInput)
/// }
/// ```
///
...
internal func invokeAsyncFunction<Promise: PromiseStruct>(
    _ body: (UnsafeMutablePointer<Promise>) -> MochiFfiErrorRef?,
    saveCancellationId: (MochiCancellationId) -> Void = { _ in }
