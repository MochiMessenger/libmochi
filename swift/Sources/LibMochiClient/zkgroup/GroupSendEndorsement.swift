...
public class GroupSendEndorsement: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_group_send_endorsement_check_valid_contents)
    }
            ...
            return slices.withUnsafeBufferPointer { slices in
                failOnError {
                    try invokeFnReturningVariableLengthSerialized {
                        mochi_group_send_endorsement_combine($0, MochiBorrowedSliceOfBuffers(base: slices.baseAddress, length: slices.count))
                    }
                }
            }
        }
        ...
        return failOnError {
            try withUnsafeBorrowedBuffer { endorsement in
                try toRemove.withUnsafeBorrowedBuffer { toRemove in
                    try invokeFnReturningVariableLengthSerialized {
                        mochi_group_send_endorsement_remove($0, endorsement, toRemove)
                    }
                }
            }
        }
    ...
    public class Token: ByteArray {
        public required init(contents: [UInt8]) throws {
            try super.init(contents, checkValid: mochi_group_send_token_check_valid_contents)
        }
        ...
        public func toFullToken(expiration: Date) -> GroupSendFullToken {
            return failOnError {
                try withUnsafeBorrowedBuffer { token in
                    try invokeFnReturningVariableLengthSerialized {
                        mochi_group_send_token_to_full_token($0, token, UInt64(expiration.timeIntervalSince1970))
                    }
                }
            }
        }
        ...
        return failOnError {
            try withUnsafeBorrowedBuffer { endorsement in
                try groupParams.withUnsafePointerToSerialized { groupParams in
                    try invokeFnReturningVariableLengthSerialized {
                        mochi_group_send_endorsement_to_token($0, endorsement, groupParams)
                    }
                }
            }
        }
