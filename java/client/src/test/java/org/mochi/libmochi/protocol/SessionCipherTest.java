...
public class SessionCipherTest extends TestCase {
  ...
  public void testMessageKeyLimits() throws Exception {
    ...
    aliceStore.storeSession(new MochiProtocolAddress("+14159999999", 1), sessions.aliceSession);
    bobStore.storeSession(new MochiProtocolAddress("+14158888888", 1), sessions.bobSession);

    SessionCipher aliceCipher =
        new SessionCipher(aliceStore, new MochiProtocolAddress("+14159999999", 1));
    SessionCipher bobCipher =
        new SessionCipher(bobStore, new MochiProtocolAddress("+14158888888", 1));
    ...
    bobCipher.decrypt(new MochiMessage(inflight.get(1000).serialize()));
    bobCipher.decrypt(new MochiMessage(inflight.get(inflight.size() - 1).serialize()));

    try {
      bobCipher.decrypt(new MochiMessage(inflight.get(0).serialize()));
  ...
  public void testDecryptAfterReset() throws Exception {
    ...
    MochiProtocolAddress aliceAddress = new MochiProtocolAddress("+14159999999", 1);
    MochiProtocolAddress bobAddress = new MochiProtocolAddress("+141588888888", 1);
    ...
    byte[] bobPlaintext = bobCipher.decrypt(new MochiMessage(message.serialize()));
    ...
    byte[] bobPlaintext2 = bobCipher.decrypt(new MochiMessage(message2.serialize()));
  ...
  public void testDecryptAfterDelete() throws Exception {
    ...
    try {
      bobCipher.decrypt(new MochiMessage(message2.serialize()));
  ...
  private void runInteraction(SessionRecord aliceSessionRecord, SessionRecord bobSessionRecord)
    ...
    aliceStore.storeSession(new MochiProtocolAddress("+14159999999", 1), aliceSessionRecord);
    bobStore.storeSession(new MochiProtocolAddress("+14158888888", 1), bobSessionRecord);
    ...
    byte[] receivedReply = aliceCipher.decrypt(new MochiMessage(reply.serialize()));
    ...
    for (int i = 0; i < aliceCiphertextMessages.size() / 2; i++) {
      byte[] receivedPlaintext =
          bobCipher.decrypt(new MochiMessage(aliceCiphertextMessages.get(i).serialize()));
    ...
    for (int i = 0; i < bobCiphertextMessages.size() / 2; i++) {
      byte[] receivedPlaintext =
          aliceCipher.decrypt(new MochiMessage(bobCiphertextMessages.get(i).serialize()));
