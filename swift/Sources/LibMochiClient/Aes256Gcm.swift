...
public struct Aes256GcmEncryptedData {
    ...
    public init(concatenated: Data) throws {
        guard concatenated.count >= Self.nonceLength + Self.authenticationTagLength else {
            throw MochiError.invalidMessage("concatenated AES-256-GCM ciphertext too short")
        }
    ...
    public func decrypt(
        ...
        guard try cipher.verifyTag(self.authenticationTag) else {
            throw MochiError.invalidMessage("failed to decrypt")
        }
        return plaintext
    }
...
public class Aes256GcmEncryption: NativeHandleOwner {
        ...
        let handle: OpaquePointer? = try key.withUnsafeBorrowedBuffer { keyBuffer in
            try nonce.withUnsafeBorrowedBuffer { nonceBuffer in
                try associatedData.withUnsafeBorrowedBuffer { adBuffer in
                    ...
                    try checkError(mochi_aes256_gcm_encryption_new(
                        &result,
                        keyBuffer,
                        nonceBuffer,
                        adBuffer
    ...
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_aes256_gcm_encryption_destroy(handle)
    }
    ...
    public func encrypt(_ message: inout Data) throws {
        try withNativeHandle { nativeHandle in
            try message.withUnsafeMutableBytes { messageBytes in
                try checkError(mochi_aes256_gcm_encryption_update(
                    nativeHandle,
                    MochiBorrowedMutableBuffer(messageBytes),
                    0,
    ...
    public func computeTag() throws -> Data {
        return try withNativeHandle { nativeHandle in
            try invokeFnReturningData {
                mochi_aes256_gcm_encryption_compute_tag($0, nativeHandle)
            }
        }
    }
}
...
public class Aes256GcmDecryption: NativeHandleOwner {
    public convenience init(
        ...
        let handle: OpaquePointer? = try key.withUnsafeBorrowedBuffer { keyBuffer in
            try nonce.withUnsafeBorrowedBuffer { nonceBuffer in
                try associatedData.withUnsafeBorrowedBuffer { adBuffer in
                    var result: OpaquePointer?
                    try checkError(mochi_aes256_gcm_decryption_new(
                        &result,
                        keyBuffer,
                        nonceBuffer,
                        adBuffer
    ...
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_aes256_gcm_decryption_destroy(handle)
    }

    public func decrypt(_ message: inout Data) throws {
        try withNativeHandle { nativeHandle in
            try message.withUnsafeMutableBytes { messageBytes in
                try checkError(mochi_aes256_gcm_decryption_update(
                    nativeHandle,
                    MochiBorrowedMutableBuffer(messageBytes),
                    0,
                    UInt32(messageBytes.count)
    ...
    public func verifyTag(_ tag: some ContiguousBytes) throws -> Bool {
        return try withNativeHandle { nativeHandle in
            try tag.withUnsafeBorrowedBuffer { tagBuffer in
                var result = false
                try checkError(mochi_aes256_gcm_decryption_verify_tag(
                    &result,
                    nativeHandle,
                    tagBuffer
                ))
