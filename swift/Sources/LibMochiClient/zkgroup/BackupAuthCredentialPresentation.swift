...
public class BackupAuthCredentialPresentation: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_backup_auth_credential_presentation_check_valid_contents)
    }
    ...
    public func verify(now: Date = Date(), serverParams: GenericServerSecretParams) throws {
        try withUnsafeBorrowedBuffer { contents in
            try serverParams.withUnsafeBorrowedBuffer { serverParams in
                try checkError(mochi_backup_auth_credential_presentation_verify(contents, UInt64(now.timeIntervalSince1970), serverParams))
            }
        }
    }
}
