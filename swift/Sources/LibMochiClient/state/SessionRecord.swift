...
public class SessionRecord: ClonableHandleOwner {
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_session_record_destroy(handle)
    }
    ...
    override internal class func cloneNativeHandle(_ newHandle: inout OpaquePointer?, currentHandle: OpaquePointer?) -> MochiFfiErrorRef? {
        return mochi_session_record_clone(&newHandle, currentHandle)
    }

    public convenience init<Bytes: ContiguousBytes>(bytes: Bytes) throws {
        let handle: OpaquePointer? = try bytes.withUnsafeBorrowedBuffer {
            var result: OpaquePointer?
            try checkError(mochi_session_record_deserialize(&result, $0))
            return result
        }
        ...
        return self.withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningArray {
                    mochi_session_record_serialize($0, nativeHandle)
                }
            }
        }
    }
    ...
    public func hasCurrentState(now: Date) -> Bool {
        ...
        self.withNativeHandle { nativeHandle in
            failOnError(mochi_session_record_has_usable_sender_chain(&result, nativeHandle, UInt64(now.timeIntervalSince1970 * 1000)))
        }
        return result
    }
    ...
    public func archiveCurrentState() {
        self.withNativeHandle { nativeHandle in
            failOnError(mochi_session_record_archive_current_state(nativeHandle))
        }
    }
    ...
    public func remoteRegistrationId() throws -> UInt32 {
        return try self.withNativeHandle { nativeHandle in
            try invokeFnReturningInteger {
                mochi_session_record_get_remote_registration_id($0, nativeHandle)
            }
        }
    }
    ...
    public func currentRatchetKeyMatches(_ key: PublicKey) throws -> Bool {
        ...
        try withNativeHandles(self, key) { sessionHandle, keyHandle in
            try checkError(mochi_session_record_current_ratchet_key_matches(&result, sessionHandle, keyHandle))
        }
        return result
    }
}
