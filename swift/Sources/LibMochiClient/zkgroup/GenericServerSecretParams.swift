...
public class GenericServerSecretParams: ByteArray {
        ...
        return failOnError {
            try randomness.withUnsafePointerToBytes { randomness in
                try invokeFnReturningVariableLengthSerialized {
                    mochi_generic_server_secret_params_generate_deterministic($0, randomness)
                }
            }
        }
    }
    ...
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_generic_server_secret_params_check_valid_contents)
    }
    ...
    public func getPublicParams() -> GenericServerPublicParams {
        return failOnError {
            try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningVariableLengthSerialized {
                    mochi_generic_server_secret_params_get_public_params($0, contents)
                }
            }
        }
    }
