...
public class Aes256Ctr32: NativeHandleOwner {
        ...
        let handle: OpaquePointer? = try key.withUnsafeBorrowedBuffer { keyBuffer in
            try nonce.withUnsafeBytes { nonceBytes in
                guard nonceBytes.count == Self.nonceLength else {
                    throw MochiError.invalidArgument("nonce must be \(Self.nonceLength) bytes (got \(nonceBytes.count))")
                }
                let initialCounter =
                ...
                var nonceBufferWithoutCounter = MochiBorrowedBuffer(nonceBytes)
                nonceBufferWithoutCounter.length -= 4
                var result: OpaquePointer?
                try checkError(mochi_aes256_ctr32_new(
                    &result,
                    keyBuffer,
    ...
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_aes256_ctr32_destroy(handle)
    }
    ...
    public func process(_ message: inout Data) throws {
        try withNativeHandle { nativeHandle in
            try message.withUnsafeMutableBytes { messageBytes in
                try checkError(mochi_aes256_ctr32_process(
                    nativeHandle,
