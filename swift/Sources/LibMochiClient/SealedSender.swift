...
public func sealedSenderEncrypt<Bytes: ContiguousBytes>(
    ...
    let ciphertextMessage = try mochiEncrypt(
        message: message,
        for: address,
        sessionStore: sessionStore,
...
public class UnidentifiedSenderMessageContent: NativeHandleOwner {
    public struct ContentHint: RawRepresentable, Hashable {
        ...
        internal init(_ knownType: MochiContentHint) {
            self.init(rawValue: UInt32(knownType.rawValue))
        }

        public static var `default`: Self {
            return Self(MochiContentHintDefault)
        }

        public static var resendable: Self {
            return Self(MochiContentHintResendable)
        }

        public static var implicit: Self {
            return Self(MochiContentHintImplicit)
        }
    }
    ...
    public convenience init<Bytes: ContiguousBytes>(
        ...
        try sealedSenderMessage.withUnsafeBorrowedBuffer { messageBuffer in
            try withIdentityKeyStore(identityStore, context) { ffiIdentityStore in
                try checkError(
                    mochi_sealed_session_cipher_decrypt_to_usmc(
                        &result,
                        messageBuffer,
                        ffiIdentityStore
                    ))
    ...
    public convenience init<GroupIdBytes: ContiguousBytes>(
        ...
        try withNativeHandles(message, sender) { messageHandle, senderHandle in
            try groupId.withUnsafeBorrowedBuffer { groupIdBuffer in
                try checkError(
                    mochi_unidentified_sender_message_content_new(
                        &result,
                        messageHandle,
                        senderHandle,
    ...
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_unidentified_sender_message_content_destroy(handle)
    }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningNativeHandle {
                    mochi_unidentified_sender_message_content_get_sender_cert($0, nativeHandle)
                }
            }
        }
    }
        ...
        let rawType = withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    mochi_unidentified_sender_message_content_get_msg_type($0, nativeHandle)
                }
            }
        }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningArray {
                    mochi_unidentified_sender_message_content_get_contents($0, nativeHandle)
                }
            }
        }
    }
    ...
    public var groupId: [UInt8]? {
        let result = withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningArray {
                    mochi_unidentified_sender_message_content_get_group_id_or_empty($0, nativeHandle)
                }
            }
        }
        if result.isEmpty {
    ...
    public var contentHint: ContentHint {
        let rawHint = withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    mochi_unidentified_sender_message_content_get_content_hint($0, nativeHandle)
                }
            }
        }
...
public func sealedSenderEncrypt(
    ...
    return try withNativeHandles(recipient, content) { recipientHandle, contentHandle in
        try withIdentityKeyStore(identityStore, context) { ffiIdentityStore in
            try invokeFnReturningArray {
                mochi_sealed_session_cipher_encrypt(
                    $0,
                    recipientHandle,
                    contentHandle,
                    ffiIdentityStore
    ...
    return try withExtendedLifetime((recipients, sessions)) {
        ...
        return try content.withNativeHandle { contentHandle in
            try recipientHandles.withUnsafeBufferPointer { recipientHandles in
                let recipientHandlesBuffer = MochiBorrowedSliceOfProtocolAddress(base: recipientHandles.baseAddress, length: recipientHandles.count)
                return try sessionHandles.withUnsafeBufferPointer { sessionHandles in
                    let sessionHandlesBuffer = MochiBorrowedSliceOfSessionRecord(base: sessionHandles.baseAddress, length: sessionHandles.count)
                    return try ServiceId.concatenatedFixedWidthBinary(excludedRecipients).withUnsafeBorrowedBuffer { excludedRecipientsBuffer in
                        try withIdentityKeyStore(identityStore, context) { ffiIdentityStore in
                            try invokeFnReturningArray {
                                mochi_sealed_sender_multi_recipient_encrypt(
                                    $0,
...
internal func sealedSenderMultiRecipientMessageForSingleRecipient(_ message: [UInt8]) throws -> [UInt8] {
    return try message.withUnsafeBorrowedBuffer { message in
        try invokeFnReturningArray {
            mochi_sealed_sender_multi_recipient_message_for_single_recipient($0, message)
        }
    }
}
...
public func sealedSenderDecrypt<Bytes: ContiguousBytes>(
                ...
                try withIdentityKeyStore(identityStore, context) { ffiIdentityStore in
                    try withPreKeyStore(preKeyStore, context) { ffiPreKeyStore in
                        try withSignedPreKeyStore(signedPreKeyStore, context) { ffiSignedPreKeyStore in
                            try invokeFnReturningArray {
                                mochi_sealed_session_cipher_decrypt(
                                    $0,
                                    &senderE164,
                                    &senderUUID,
                                    &senderDeviceId,
    ...
    defer {
        mochi_free_string(senderE164)
        mochi_free_string(senderUUID)
    }
