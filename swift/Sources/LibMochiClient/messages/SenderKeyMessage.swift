//
// Copyright 2020-2022 Mochi Messenger, LLC.
// SPDX-License-Identifier: AGPL-3.0-only
//

import Foundation
import MochiFfi

public class SenderKeyMessage: NativeHandleOwner {
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_sender_key_message_destroy(handle)
    }

    public convenience init<Bytes: ContiguousBytes>(bytes: Bytes) throws {
        var result: OpaquePointer?
        try bytes.withUnsafeBorrowedBuffer {
            try checkError(mochi_sender_key_message_deserialize(&result, $0))
        }
        self.init(owned: result!)
    }

    public var distributionId: UUID {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningUuid {
                    mochi_sender_key_message_get_distribution_id($0, nativeHandle)
                }
            }
        }
    }

    public var chainId: UInt32 {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    mochi_sender_key_message_get_chain_id($0, nativeHandle)
                }
            }
        }
    }

    public var iteration: UInt32 {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningInteger {
                    mochi_sender_key_message_get_iteration($0, nativeHandle)
                }
            }
        }
    }

    public func serialize() -> [UInt8] {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningArray {
                    mochi_sender_key_message_serialize($0, nativeHandle)
                }
            }
        }
    }

    public var ciphertext: [UInt8] {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningArray {
                    mochi_sender_key_message_get_cipher_text($0, nativeHandle)
                }
            }
        }
    }

    public func verifySignature(against key: PublicKey) throws -> Bool {
        var result = false
        try withNativeHandles(self, key) { messageHandle, keyHandle in
            try checkError(mochi_sender_key_message_verify_signature(&result, messageHandle, keyHandle))
        }
        return result
    }
}
