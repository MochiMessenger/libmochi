...
public class GroupSendEndorsementsResponse: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_group_send_endorsements_response_check_valid_contents)
    }
                ...
                try keyPair.withUnsafeBorrowedBuffer { keyPair in
                    try randomness.withUnsafePointerToBytes { randomness in
                        try invokeFnReturningVariableLengthSerialized {
                            mochi_group_send_endorsements_response_issue_deterministic($0, concatenated, keyPair, randomness)
                        }
                    }
                }
            }
        ...
        let expirationSinceEpoch: UInt64 = failOnError {
            try withUnsafeBorrowedBuffer { response in
                try invokeFnReturningInteger {
                    mochi_group_send_endorsements_response_get_expiration($0, response)
                }
            }
        }
    ...
    ///
    /// - Throws: ``MochiError/verificationFailed(_:)`` if the endorsements are not valid for any
    ///   reason
    public func receive(
                    ...
                    try groupParams.withUnsafePointerToSerialized { groupParams in
                        try serverParams.withNativeHandle { serverParams in
                            try invokeFnReturningBytestringArray {
                                mochi_group_send_endorsements_response_receive_and_combine_with_service_ids($0, response, groupMembers, localUser, UInt64(now.timeIntervalSince1970), groupParams, serverParams)
                            }
                        }
                    }
                }
        ...
        let rawEndorsements = try withUnsafeBorrowedBuffer { response in
            try groupMembers.flatMap { $0.serialize() }.withUnsafeBorrowedBuffer { groupMembers in
                try localUser.withUnsafeBorrowedBuffer { localUser in
                    try serverParams.withNativeHandle { serverParams in
                        try invokeFnReturningBytestringArray {
                            mochi_group_send_endorsements_response_receive_and_combine_with_ciphertexts($0, response, groupMembers, localUser, UInt64(now.timeIntervalSince1970), serverParams)
                        }
                    }
                }
            }
