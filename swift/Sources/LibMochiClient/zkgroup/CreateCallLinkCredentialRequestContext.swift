...
public class CreateCallLinkCredentialRequestContext: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_create_call_link_credential_request_context_check_valid_contents)
    }
            ...
            try roomId.withUnsafeBorrowedBuffer { roomId in
                try randomness.withUnsafePointerToBytes { randomness in
                    try invokeFnReturningVariableLengthSerialized {
                        mochi_create_call_link_credential_request_context_new_deterministic($0, roomId, randomness)
                    }
                }
            }
        }
    ...
    public func getRequest() -> CreateCallLinkCredentialRequest {
        return failOnError {
            try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningVariableLengthSerialized {
                    mochi_create_call_link_credential_request_context_get_request($0, contents)
                }
            }
        }
    }
            ...
            try response.withUnsafeBorrowedBuffer { response in
                try userId.withPointerToFixedWidthBinary { userId in
                    try params.withUnsafeBorrowedBuffer { params in
                        try invokeFnReturningVariableLengthSerialized {
                            mochi_create_call_link_credential_request_context_receive_response($0, contents, response, userId, params)
                        }
                    }
                }
            }
