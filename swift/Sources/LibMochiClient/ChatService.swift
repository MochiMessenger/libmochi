...
public enum IpType: UInt8 {
    // Must be kept in sync with libmochi-net's IpType.
...
public class ChatService: NativeHandleOwner {
    ...
    public struct Response: Equatable {
        ...
        // Exposed for testing.
        internal init(consuming rawResponse: MochiFfiChatResponse) throws {
    ...
    public struct DebugInfo: Equatable {
        ...
        internal init(consuming rawDebugInfo: MochiFfiChatServiceDebugInfo) {
    ...
    internal init(tokioAsyncContext: TokioAsyncContext, connectionManager: ConnectionManager, username: String, password: String) {
        ...
        connectionManager.withNativeHandle { connectionManager in
            failOnError(mochi_chat_service_new(&handle, connectionManager, username, password))
        }
    ...
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_chat_destroy(handle)
    }
        ...
        self.tokioAsyncContext.withNativeHandle { tokioAsyncContext in
            withNativeHandle { chatService in
                if let listener {
                    ...
                    failOnError(mochi_chat_server_set_listener(tokioAsyncContext, chatService, &listenerStruct))
                } else {
                    failOnError(mochi_chat_server_set_listener(tokioAsyncContext, chatService, nil))
                }
            }
        }
    }
    ...
    ///
    /// - Throws: ``MochiError/appExpired(_:)`` if the current app version is too old (as judged by
    ///   the server).
    /// - Throws: ``MochiError/deviceDeregistered(_:)`` if the current device has been deregistered
    ///   or delinked.
    /// - Throws: Other ``MochiError``s for other kinds of failures.
    @discardableResult
    public func connectAuthenticated() async throws -> DebugInfo {
        let rawDebugInfo = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatService in
                mochi_chat_service_connect_auth(promise, tokioAsyncContext, chatService)
            }
        }
    ...
    public func connectUnauthenticated() async throws -> DebugInfo {
        let rawDebugInfo = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatService in
                mochi_chat_service_connect_unauth(promise, tokioAsyncContext, chatService)
            }
        }
        return DebugInfo(consuming: rawDebugInfo)
    }
    ...
    public func disconnect() async throws {
        _ = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatService in
                mochi_chat_service_disconnect(promise, tokioAsyncContext, chatService)
            }
        }
    }
    ...
    ///
    /// - Throws: ``MochiError/chatServiceInactive(_:)`` if you haven't called ``connectUnauthenticated()``
    ...
    public func unauthenticatedSend(_ request: Request) async throws -> Response {
        ...
        let rawResponse: MochiFfiChatResponse = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatService in
                internalRequest.withNativeHandle { request in
                    mochi_chat_service_unauth_send(promise, tokioAsyncContext, chatService, request, timeoutMillis)
                }
            }
        }
    ...
    public func unauthenticatedSendAndDebug(_ request: Request) async throws -> (Response, DebugInfo) {
        let internalRequest = try InternalRequest(request)
        let timeoutMillis = request.timeoutMillis
        let rawResponse: MochiFfiResponseAndDebugInfo = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatService in
                internalRequest.withNativeHandle { request in
                    mochi_chat_service_unauth_send_and_debug(promise, tokioAsyncContext, chatService, request, timeoutMillis)
                }
            }
        }
    ...
    ///
    /// - Throws: ``MochiError/chatServiceInactive(_:)`` if you haven't called ``connectAuthenticated()``
    ...
    public func authenticatedSend(_ request: Request) async throws -> Response {
        ...
        let rawResponse: MochiFfiChatResponse = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatService in
                internalRequest.withNativeHandle { request in
                    mochi_chat_service_auth_send(promise, tokioAsyncContext, chatService, request, timeoutMillis)
                }
            }
        }
        return try Response(consuming: rawResponse)
    ...
    public func authenticatedSendAndDebug(_ request: Request) async throws -> (Response, DebugInfo) {
        ...
        let rawResponse: MochiFfiResponseAndDebugInfo = try await self.tokioAsyncContext.invokeAsyncFunction { promise, tokioAsyncContext in
            withNativeHandle { chatService in
                internalRequest.withNativeHandle { request in
                    mochi_chat_service_auth_send_and_debug(promise, tokioAsyncContext, chatService, request, timeoutMillis)
                }
            }
        }
        return (try Response(consuming: rawResponse.response), DebugInfo(consuming: rawResponse.debug_info))
        ...
        convenience init(_ request: Request) throws {
            ...
            if let body = request.body {
                try body.withUnsafeBorrowedBuffer { body in
                    try checkError(mochi_http_request_new_with_body(&handle, request.method, request.pathAndQuery, body))
                }
            } else {
                try checkError(mochi_http_request_new_without_body(&handle, request.method, request.pathAndQuery))
            }
            // Make sure we clean up the handle if there are any errors adding headers.
            self.init(owned: handle!)

            for (name, value) in request.headers {
                try checkError(mochi_http_request_add_header(handle, name, value))
            }
        }

        override class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
            return mochi_http_request_destroy(handle)
        }
    }
}

extension MochiFfiChatResponse {
    ...
    internal mutating func free() {
        mochi_free_string(message)
        mochi_free_list_of_strings(headers)
        mochi_free_buffer(body.base, body.length)
        // Zero out all the fields to be sure they won't be reused.
        self = .init()
    }
}

extension MochiFfiChatServiceDebugInfo {
    fileprivate mutating func free() {
        mochi_free_string(connection_info)
        // Zero out all the fields to be sure they won't be reused.
        self = .init()
    }
}
