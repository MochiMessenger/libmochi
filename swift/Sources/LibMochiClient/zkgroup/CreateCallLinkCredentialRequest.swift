...
public class CreateCallLinkCredentialRequest: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_create_call_link_credential_request_check_valid_contents)
    }
                    ...
                    try params.withUnsafeBorrowedBuffer { params in
                        try randomness.withUnsafePointerToBytes { randomness in
                            try invokeFnReturningVariableLengthSerialized {
                                mochi_create_call_link_credential_request_issue_deterministic($0, contents, userId, UInt64(timestamp.timeIntervalSince1970), params, randomness)
                            }
                        }
                    }
                }
