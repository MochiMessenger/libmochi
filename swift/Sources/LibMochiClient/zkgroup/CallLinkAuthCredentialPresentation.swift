...
public class CallLinkAuthCredentialPresentation: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_call_link_auth_credential_presentation_check_valid_contents)
    }
        ...
        try withUnsafeBorrowedBuffer { contents in
            try serverParams.withUnsafeBorrowedBuffer { serverParams in
                try callLinkParams.withUnsafeBorrowedBuffer { callLinkParams in
                    try checkError(mochi_call_link_auth_credential_presentation_verify(contents, UInt64(now.timeIntervalSince1970), serverParams, callLinkParams))
                }
            }
        }
    }
        ...
        return failOnError {
            try withUnsafeBorrowedBuffer { contents in
                try invokeFnReturningSerialized {
                    mochi_call_link_auth_credential_presentation_get_user_id($0, contents)
                }
            }
        }
    }
