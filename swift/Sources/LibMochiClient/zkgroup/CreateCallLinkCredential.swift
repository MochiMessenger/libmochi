...
public class CreateCallLinkCredential: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_create_call_link_credential_check_valid_contents)
    }
                            ...
                            try callLinkParams.withUnsafeBorrowedBuffer { callLinkParams in
                                try randomness.withUnsafePointerToBytes { randomness in
                                    try invokeFnReturningVariableLengthSerialized {
                                        mochi_create_call_link_credential_present_deterministic($0, contents, roomId, userId, serverParams, callLinkParams, randomness)
                                    }
                                }
                            }
                        }
