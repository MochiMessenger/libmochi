...
public protocol MochiInputStream: AnyObject {
...
/// An error thrown by `MochiInputStreamAdapter`.
public enum MochiInputStreamError: Error {
    /// The end of the input stream was reached while attempting to `skip()`.
    case unexpectedEof
}

extension FileHandle: MochiInputStream {
...
/// An adapter implementing `MochiInputStream` for any `Collection<UInt8>`.
public class MochiInputStreamAdapter<Inner>: MochiInputStream where Inner: Collection<UInt8> {
    ...
    public func skip(by amount: UInt64) throws {
        if amount > UInt64(self.inner.count) {
            throw MochiInputStreamError.unexpectedEof
        }
