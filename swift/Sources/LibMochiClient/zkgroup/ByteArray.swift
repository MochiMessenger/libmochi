//
// Copyright 2019-2022 Mochi Messenger, LLC.
...
public class ByteArray {
    ...
    init(_ newContents: [UInt8], checkValid: (MochiBorrowedBuffer) -> MochiFfiErrorRef?) throws {
    ...
    init(newContents: [UInt8], expectedLength: Int, unrecoverable: Bool = false) throws {
        if newContents.count != expectedLength {
            throw MochiError.invalidType("\(type(of: self)) uses \(expectedLength) bytes, but tried to deserialize from an array of \(newContents.count) bytes")
        }
        self.contents = newContents
    }
    ...
    func withUnsafePointerToSerialized<Serialized, Result>(_ callback: (UnsafePointer<Serialized>) throws -> Result) throws -> Result {
        ...
        return try self.contents.withUnsafeBytes { buffer in
            ...
            guard expectedSize == buffer.count else {
                throw MochiError.invalidType("\(type(of: self)) uses \(buffer.count) bytes, but was passed to a callback that uses \(expectedSize) bytes")
            }
    ...
    func withUnsafeBorrowedBuffer<Result>(_ callback: (MochiBorrowedBuffer) throws -> Result) throws -> Result {
