...
public class GroupSendDerivedKeyPair: ByteArray {
    public required init(contents: [UInt8]) throws {
        try super.init(contents, checkValid: mochi_group_send_derived_key_pair_check_valid_contents)
    }
        ...
        return failOnError {
            try params.withNativeHandle { params in
                try invokeFnReturningVariableLengthSerialized {
                    mochi_group_send_derived_key_pair_for_expiration($0, UInt64(expiration.timeIntervalSince1970), params)
                }
            }
        }
    }
