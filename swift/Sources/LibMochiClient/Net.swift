...
/// Entry point for interacting with Mochi remote services.
public class Net {
    /// The Mochi environment to use when connecting to remote services.
    ///
    ...
    public enum Environment: UInt8 {
        ...
        /// Mochi's staging environment.
        case staging = 0

        /// Mochi's production environment.
        case production = 1
    }
    ...
    /// Creates a new `Net` instance that enables interacting with services in the given Mochi environment.
    ...
    ///
    /// - Throws: On error, throws a ``MochiError``. Expected error cases are
    ///   `MochiError.networkError` for a network-level connectivity issue,
    ///   `MochiError.networkProtocolError` for a CDSI or attested connection protocol issue,
    ///   `MochiError.rateLimitedError` with the amount of time to wait before trying again.
    ///
    /// ## Example:
    ///
    /// ```swift
        ...
        let handle: OpaquePointer = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.connectionManager.withNativeHandle { connectionManager in
                request.withNativeHandle { request in
                    mochi_cdsi_lookup_new(promise, asyncContext, connectionManager, auth.username, auth.password, request)
                }
            }
        }
...
extension Auth {
    ...
    internal init(username: String, enclaveSecret: String) throws {
        let otp = try invokeFnReturningString {
            mochi_create_otp_from_base64($0, username, enclaveSecret)
        }
...
public class CdsiLookupRequest: NativeHandleOwner {
    ...
    private convenience init() {
        ...
        try! checkError(mochi_lookup_request_new(&handle))
    ...
    ///
    /// - Throws: a ``MochiError`` if any of the arguments are invalid,
    ...
    public convenience init(
        ...
        try self.withNativeHandle { handle in
            for e164 in e164s {
                try checkError(mochi_lookup_request_add_e164(handle, e164))
            }

            for prevE164 in prevE164s {
                try checkError(mochi_lookup_request_add_previous_e164(handle, prevE164))
            }
            ...
            for aciAndAccessKey in acisAndAccessKeys {
                ...
                try aci.withPointerToFixedWidthBinary { aci in
                    try accessKey.withUnsafeBorrowedBuffer { accessKey in
                        try checkError(mochi_lookup_request_add_aci_and_access_key(handle, aci, accessKey))
                    }
                }
            }
            ...
            if let token = token {
                try token.withUnsafeBorrowedBuffer { token in
                    try checkError(mochi_lookup_request_set_token(handle, token))
                }
                self.hasToken = true
            }

            try checkError(mochi_lookup_request_set_return_acis_without_uaks(handle, returnAcisWithoutUaks))
        }
    }

    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        mochi_lookup_request_destroy(handle)
    }
}
...
public class CdsiLookup {
    class NativeCdsiLookup: NativeHandleOwner {
        override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
            mochi_cdsi_lookup_destroy(handle)
        }
    }
        ...
        failOnError {
            try self.native.withNativeHandle { handle in
                try invokeFnReturningData {
                    mochi_cdsi_lookup_token($0, handle)
                }
            }
        }
    }
    ...
    ///
    /// - Throws: ``MochiError`` if the request fails for any reason, including
    ///   `MochiError.networkError` for a network-level connectivity issue,
    ///   `MochiError.networkProtocolError` for a CDSI or attested connection protocol issue.
    public func complete() async throws -> CdsiLookupResponse {
        let response: MochiFfiCdsiLookupResponse = try await self.asyncContext.invokeAsyncFunction { promise, asyncContext in
            self.native.withNativeHandle { handle in
                mochi_cdsi_lookup_complete(promise, asyncContext, handle)
            }
        }
...
public class LookupResponseEntryList: Collection {
    ...
    init(owned: MochiOwnedBufferOfFfiCdsiLookupResponseEntry) {
        self.owned = UnsafeMutableBufferPointer(start: owned.base, count: Int(owned.length))
    }

    deinit {
        mochi_free_lookup_response_entry_list(MochiOwnedBufferOfFfiCdsiLookupResponseEntry(base: self.owned.baseAddress, length: self.owned.count))
    }
...
public typealias CdsiLookupResponseEntry = MochiFfiCdsiLookupResponseEntry
...
internal class ConnectionManager: NativeHandleOwner {
    convenience init(env: Net.Environment, userAgent: String) {
        var handle: OpaquePointer?
        failOnError(mochi_connection_manager_new(&handle, env.rawValue, userAgent))
        self.init(owned: handle!)
    }
    ...
    internal func setProxy(host: String, port: UInt16) throws {
        try self.withNativeHandle {
            ...
            try checkError(mochi_connection_manager_set_proxy($0, host, Int32(port)))
        }
    }

    internal func clearProxy() {
        self.withNativeHandle {
            failOnError(mochi_connection_manager_clear_proxy($0))
        }
    }

    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        mochi_connection_manager_destroy(handle)
    }
}
