...
public class PublicKey: ClonableHandleOwner {
    public convenience init<Bytes: ContiguousBytes>(_ bytes: Bytes) throws {
        let handle: OpaquePointer? = try bytes.withUnsafeBorrowedBuffer {
            ...
            try checkError(mochi_publickey_deserialize(&result, $0))
            return result
        }
    ...
    override internal class func destroyNativeHandle(_ handle: OpaquePointer) -> MochiFfiErrorRef? {
        return mochi_publickey_destroy(handle)
    }
    ...
    override internal class func cloneNativeHandle(_ newHandle: inout OpaquePointer?, currentHandle: OpaquePointer?) -> MochiFfiErrorRef? {
        return mochi_publickey_clone(&newHandle, currentHandle)
    }
        ...
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningArray {
                    mochi_publickey_get_public_key_bytes($0, nativeHandle)
                }
            }
        }
    }
    ...
    public func serialize() -> [UInt8] {
        return withNativeHandle { nativeHandle in
            failOnError {
                try invokeFnReturningArray {
                    mochi_publickey_serialize($0, nativeHandle)
                }
            }
        }
    }
        ...
        try withNativeHandle { nativeHandle in
            try message.withUnsafeBorrowedBuffer { messageBuffer in
                try signature.withUnsafeBorrowedBuffer { signatureBuffer in
                    try checkError(mochi_publickey_verify(&result, nativeHandle, messageBuffer, signatureBuffer))
                }
            }
        }
        return result
    ...
    public func compare(_ other: PublicKey) -> Int32 {
        ...
        withNativeHandles(self, other) { selfHandle, otherHandle in
            failOnError(mochi_publickey_compare(&result, selfHandle, otherHandle))
        }
        return result
    }
}
